...
function sendRequestGet(){
	if(!document.getElementById('sync').checked){
		dhtmlxAjax.get("php/processPosBigScreen1.php?"+encodeURI(document.getElementById('params').value),outputResponse);
		alert("Request Sent");
	}else{
		var loader = dhtmlxAjax.getSync("php/processPosBigScreen1.php?"+encodeURI(document.getElementById('params').value));
		alert("Request Sent");
...
//				dhtmlxAjax.post("php/process.php","uid="+E.userid+"&key="+key+"&value="+value,outputResponse);
		// uid =
		var url = "./TriCompExpHandlerPosBigScreen1.js";
		var data = {uid:E.userid, key: key, value:value};
		console.log(url);
		$.ajax({
			type: "POST",
			// method: "POST",
